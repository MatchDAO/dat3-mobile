<template>
  <view class="container">
    ...
    <view class="top-tit">{{$t('login.login')}}</view>
    <view class="login-form">
      ...
      <view class="btn-tip" @click="goInvite">{{$t('login.tip')}}</view>
    </view>
    <!--  -->
    <w-popup-confirm
      ref="popup"
...
import { afterSayHi } from "@/api/message";
import { msgType, autoMsgPrefix } from '@/config/message';
...
export default {
  ...
  methods: {
    autoSendMsg(userInfo) {
      try {
        const sayHiList = JSON.parse(userInfo.sayHi) || [];
        if (sayHiList.length) {
          sayHiList.forEach(sayHiItem => {
            let id = WebIM.conn.getUniqueId();
            let msg = new WebIM.message(msgType.TEXT, id);
            msg.set({
              msg: autoMsgPrefix + sayHiItem.sayHi,
              to: sayHiItem.cuserCode,
              // roomType: false,
              chatType: 'singleChat',
              success() {
                afterSayHi({
                  msgId: sayHiItem.msgId,
                  userCode: sayHiItem.userCode,
                  cuserCode: sayHiItem.cuserCode
                }).then(res => {
                })
              },
              fail(id, serverMsgId) {
        ...
        if (userInfo.firstLogin) {
          ...
          if (!userInfo.address) {
            try {
              ...
              const walletInfo = walletJson ? JSON.parse(walletJson) : {};
              if (walletInfo.address) {
                updateAddress({ address: walletInfo.address });
              }
            } catch (error) {
